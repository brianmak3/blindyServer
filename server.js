...
// connect to the database
 mongoose.connect('mongodb://127.0.0.1/blindy');
...
io.on('connection', socket => {
	...
	let checkUser = () => {
		for (let i in users) {
			console.log(socket.id +' '+users[i].socket);
	...
	socket.on('login', authUser => {
		...
		Users.findOne({
			...
			if(err)
			 ...
			 else if(!re.validPassword(authUser.password))
				error = "You have submitted an invalid password."
			 if(error)
				return socket.emit('login_error', {
					action: 'IndexResponse',
					title: 'login',
					error: error
				}, null);
			if(re)
	...
	socket.on('appData', data => {
		switch(data.title){
			...
			var inputs = data.inputs;
			 Users.findOne({'phone': data.countryCode+''+inputs[2].value}, function(err, user){
			 	if(err)
			 		throw err;
			 	else if(user){
        				socketEmit(socket, {
        					action: 'IndexResponse',
        					title: data.title,
        					error: "Sorry, the phone number is already registered" 
        				}, null);
        			}
			 	else{
			 		 var inputs = data.inputs;
                     var newUser = new Users();
                        newUser.name = Date.now()+'_'+inputs[1].value;
					    newUser.email = inputs[0].value;
					    newUser.profile = false;
					    newUser.username = inputs[1].value;
					    newUser.phone = data.countryCode+''+inputs[2].value;
					    newUser.password = newUser.generatHarsh(inputs[3].value);
					    newUser.image = 'assets/imgs/avatar.jpg';
					    newUser.images = [{
					    	date: Date.now(),
					    	url: newUser.image 
					    }]
			...
			break;
			case 'settingsUpdate':
			Users.updateOne({'phone': data.user}, {$set: {settings: data.settings, profile: true}}, function(err, res){
				if(err)
					throw err;
				else socketEmit(socket, {action: 'settingsUpdate'}, null);
			})
			break;
			case 'searchRequest':
			var criteria;
			 var gender = (data.settings.gender == 'Male' ? 'Female': 'Male');
			 if(data.type == 'random'){
			 	  if(data.settings.orientation == 'Straight')
			 	     criteria = {phone: {$ne: data.user}, 'settings.gender': gender }
			 	 else 
			 	     criteria = {phone: {$ne: data.user}, 'settings.gender': data.settings.gender}
			 }else{
			 	     criteria = {phone: {$ne: data.user}, 'settings': data.settings}
			 }
			 Users.aggregate([
				 	    { $match: criteria},
				 	    { $project: {_id: 0, username: 1, 'settings.age': 1,'settings.gender': 1,'settings.orientation': 1, images: 1} },
				 	    { $sample: { size: 1 } },
			 	    ]
